...
func NewPoCMiner(name string, allowSolo bool, chain Chain, syncManager SyncManager, sk spacekeeper.SpaceKeeper, newBlockCh chan *wire.Hash, payoutAddresses []massutil.Address) *PoCMiner {
	m := &PoCMiner{
		allowSolo:       allowSolo,
		chain:           chain,
		syncManager:     syncManager,
...
func (m *PoCMiner) solveBlock(payoutAddress massutil.Address, quit chan struct{}) (*wire.MsgBlock, massutil.Amount, error) {
	...
	// Step 7: get signature for poc hash
	logging.CPrint(logging.INFO, "Step 7: get signature for poc hash")
	pocHash, err := block.Header.PoCHash()
	if err != nil {
		return failure(err)
	}
	block.Header.Signature, err = m.SpaceKeeper.SignHash(tProof.proof.SpaceID, pocHash)
	if err != nil {
		return failure(err)
	}

	logging.CPrint(logging.INFO, "Step 8: return")
