...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := pocec.S256().ScalarBaseMult(k.key)
		pubKey := pocec.PublicKey{Curve: pocec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(pocec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilx, ily := pocec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := pocec.ParsePubKey(k.key, pocec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := pocec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := pocec.PublicKey{Curve: pocec.S256(), X: childX, Y: childY}
		childKey = pk.SerializeCompressed()
	}

	// The fingerprint of the parent for the derived child is the first 4
	// bytes of the RIPEMD160(SHA256(parentPubKey)).
	parentFP := massutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a pocec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*pocec.PublicKey, error) {
	return pocec.ParsePubKey(k.pubKeyBytes(), pocec.S256())
}

// ECPrivKey converts the extended key to a pocec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*pocec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := pocec.PrivKeyFromBytes(pocec.S256(), k.key)
	return privKey, nil
}

// Address converts the extended key to a standard bitcoin pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *config.Params) (*massutil.AddressPubKeyHash, error) {
	pkHash := massutil.Hash160(k.pubKeyBytes())
	return massutil.NewAddressPubKeyHash(pkHash, net)
}
...
func (k *ExtendedKey) String() string {
	...
	checkSum := wire.DoubleHashB(serializedBytes)[:4]
...
func (k *ExtendedKey) IsForNet(net *config.Params) bool {
...
func (k *ExtendedKey) SetNet(net *config.Params) {
	if k.isPrivate {
...
func NewMaster(seed []byte, net *config.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(pocec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	expectedCheckSum := wire.DoubleHashB(payload)[:4]
	...
	if isPrivate {
		...
		if keyNum.Cmp(pocec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := pocec.ParsePubKey(keyData, pocec.S256())
		if err != nil {
			return nil, err
		}
	}
...
func (k *ExtendedKey) PrivKey() ([]byte, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}
	return k.key, nil
