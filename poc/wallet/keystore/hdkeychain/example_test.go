...
func ExampleNewMaster() {
	...
	key, err := NewMaster(seed, config.ChainParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_defaultWalletLayout() {
	...
	// This gives the path:
	//   m
	masterKey, err := NewKeyFromString(master)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	//   m/0H
	acct0, err := masterKey.Child(HardenedKeyStart + 0)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	acct0ExtAddr, err := acct0Ext10.Address(config.ChainParams)
	if err != nil {
		fmt.Println(err)
		return
	}
	acct0IntAddr, err := acct0Int0.Address(config.ChainParams)
	if err != nil {
		fmt.Println(err)
		return
	}
