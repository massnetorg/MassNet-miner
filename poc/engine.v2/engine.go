...
type WorkSpaceProof struct {
	SpaceID   string
	Proof     *chiapos.ProofOfSpace
	PublicKey *chiapos.G1Element
	Ordinal   int64 // indicates proof index
	Error     error
}

func (p *WorkSpaceProof) PlotPublicKey() []byte {
	return p.Proof.PlotPublicKey.SerializeCompressed()
}

func (p *WorkSpaceProof) ProofType() poc.ProofType {
	return poc.ProofTypeChia
}

func (p *WorkSpaceProof) ProofBitLength() int {
	return int(p.Proof.KSize)
}

func (p *WorkSpaceProof) ChiaPoolPublicKey() []byte {
	return p.Proof.PoolPublicKey.SerializeCompressed()
}

func (p *WorkSpaceProof) ChiaPlotID() [32]byte {
	plotID, _ := p.Proof.GetID()
	return plotID
}
...
func (q *WorkSpaceQuality) PlotPublicKey() []byte {
	return q.PublicKey.SerializeCompressed()
}

func (q *WorkSpaceQuality) ProofType() poc.ProofType {
	return poc.ProofTypeChia
}

func (q *WorkSpaceQuality) ProofBitLength() int {
	return int(q.KSize)
}

func (q *WorkSpaceQuality) ChiaPoolPublicKey() []byte {
	return q.PoolPublicKey.SerializeCompressed()
}

func (q *WorkSpaceQuality) ChiaPlotID() [32]byte {
	return q.PlotID
}
...
// QualityReader is the interface that wraps the basic Read-Quality method.
//
// An instance of this general case is that a QualityReader returning
// a non-nil WorkSpaceQuality at the end of the input stream may
// return either err == io.EOF or err == nil. The next Read should
// return nil, io.EOF.
type QualityReader interface {
	Read() (*WorkSpaceQuality, error)
}

var (
	// ErrQualityIOTimeout indicates quality read/write operation timeout
	ErrQualityIOTimeout = errors.New("quality i/o timeout")
)

// QualityRW implements QualityReader and provides a way that servers can writes WorkSpaceQualities.
type QualityRW struct {
	l      sync.Mutex
	ctx    context.Context
	ch     chan *WorkSpaceQuality
	closed bool
}

// NewQualityRW generates a new QualityRW instance.
func NewQualityRW(ctx context.Context, bufSize int) *QualityRW {
	qrw := &QualityRW{
		ctx: ctx,
		ch:  make(chan *WorkSpaceQuality, bufSize),
	}
	go func() {
		<-qrw.ctx.Done()
		qrw.Close()
...
// Read implements QualityReader.
func (qrw *QualityRW) Read() (*WorkSpaceQuality, error) {
	wsq, ok := <-qrw.ch
	var err error
	if !ok && wsq == nil {
		err = io.EOF
...
// Write provides a way that servers can writes WorkSpaceQualities.
//
// Write is concurrent safe, and returns an error ErrQualityIOTimeout if p.ctx is Done.
func (qrw *QualityRW) Write(wsq *WorkSpaceQuality) error {
	qrw.l.Lock()
	defer qrw.l.Unlock()
	if qrw.closed {
		return ErrQualityIOTimeout
	}
	qrw.ch <- wsq
	return nil
}

// Close closes QualityRW.
//
// Any call to Write would get ErrQualityIOTimeout.
func (qrw *QualityRW) Close() {
	qrw.l.Lock()
	if !qrw.closed {
		qrw.closed = true
		close(qrw.ch)
